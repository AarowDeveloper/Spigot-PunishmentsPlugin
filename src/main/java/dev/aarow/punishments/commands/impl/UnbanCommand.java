...
@CommandInfo(name = "unban", playerOnly = true, permission = "punishments.staff", complete = true)
public class UnbanCommand extends BaseCommand {

    @Override
    public void execute(Player player, String[] args) {
        if(args.length != 1){
            player.sendMessage(CC.translate("&7[&cCorrect Usage&7] &c/unban <player>"));
            return;
        }

        Profile targetProfile = plugin.getProfileManager().get(Bukkit.getOfflinePlayer(args[0]).getUniqueId());

        targetProfile.unpunish(player, PunishmentType.BAN);
    }

    @Override
    public List<String> complete(Player player, String[] args) {
        if(args.length == 1){
            return plugin.getProfileManager().getProfiles().stream().filter(profile -> profile.getActivePunishment(PunishmentType.BAN) != null).map(profile -> Bukkit.getOfflinePlayer(profile.getUuid()).getName()).toList();
        }
