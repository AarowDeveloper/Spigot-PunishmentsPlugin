...
@CommandInfo(name = "unmute", playerOnly = true, permission = "punishments.staff", complete = true)
public class UnmuteCommand extends BaseCommand {
    ...
    public void execute(Player player, String[] args) {
        ...
        targetProfile.unpunish(player, PunishmentType.MUTE);
    }

    @Override
    public List<String> complete(Player player, String[] args) {
        if(args.length == 1){
            return plugin.getProfileManager().getProfiles().stream().filter(profile -> profile.getActivePunishment(PunishmentType.MUTE) != null).map(profile -> Bukkit.getOfflinePlayer(profile.getUuid()).getName()).toList();
        }
